...
//----------------------------------------------------------------
//
//    Generic WindvaneInstrument Implementation
//
//----------------------------------------------------------------

WindvaneInstrument::WindvaneInstrument(wxWindow *pparent, wxWindowID id, wxString title, int cap_flag)
      :wxControl(pparent, id, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE)
{
      ...
      Connect(wxEVT_ERASE_BACKGROUND, wxEraseEventHandler(WindvaneInstrument::OnEraseBackground));
      Connect(wxEVT_PAINT, wxPaintEventHandler(WindvaneInstrument::OnPaint));
...
#ifdef __WXOSX__
      Connect(wxEVT_RIGHT_DOWN, wxMouseEventHandler(WindvaneInstrument::MouseEvent), NULL, this);
#endif      
}

void WindvaneInstrument::MouseEvent( wxMouseEvent &event )
{
...
int WindvaneInstrument::GetCapacity()
{
      return m_cap_flag;
}
void WindvaneInstrument::SetDrawSoloInPane(bool value)
{
    m_drawSoloInPane = value;
}
void WindvaneInstrument::OnEraseBackground(wxEraseEvent& WXUNUSED(evt))
{
        // intentionally empty
}

void WindvaneInstrument::OnPaint( wxPaintEvent& WXUNUSED(event) )
{
    wxBufferedPaintDC pdc( this );
    if( !pdc.IsOk() ) {
        wxLogMessage( _T("WindvaneInstrument::OnPaint() fatal: wxBufferedPaintDC.IsOk() false.") );
        return;
    }

    wxSize size = GetClientSize();
    if( size.x == 0 || size.y == 0 ) {
        wxLogMessage( _T("WindvaneInstrument::OnPaint() fatal: Zero size DC.") );
        return;
    }
...
//----------------------------------------------------------------
//
//    WindvaneInstrument_Simple Implementation
//
//----------------------------------------------------------------

WindvaneInstrument_Single::WindvaneInstrument_Single(wxWindow *pparent, wxWindowID id, wxString title, int cap_flag, wxString format)
      :WindvaneInstrument(pparent, id, title, cap_flag)
{
      m_format = format;
      m_data = _T("---");
}

wxSize WindvaneInstrument_Single::GetSize( int orient, wxSize hint )
{
      wxClientDC dc(this);
      int w;
...
void WindvaneInstrument_Single::Draw(wxGCDC* dc)
{
      wxColour cl;
#ifdef __WXMSW__
...
void WindvaneInstrument_Single::SetData(int st, double data, wxString unit)
{
      if (m_cap_flag & st){
...
//----------------------------------------------------------------
//
//    WindvaneInstrument_Position Implementation
//
//----------------------------------------------------------------

WindvaneInstrument_Position::WindvaneInstrument_Position(wxWindow *pparent, wxWindowID id, wxString title, int cap_flag1, int cap_flag2)
      :WindvaneInstrument(pparent, id, title, cap_flag1 | cap_flag2)
{
...
wxSize WindvaneInstrument_Position::GetSize( int orient, wxSize hint )
{
      wxClientDC dc(this);
      int w;
      dc.GetTextExtent(m_title, &w, &m_TitleHeight, 0, 0, g_pFontTitle);
...
void WindvaneInstrument_Position::Draw(wxGCDC* dc)
{
      wxColour cl;
...
void WindvaneInstrument_Position::SetData(int st, double data, wxString unit)
{
      if (st == m_cap_flag1)
      {
