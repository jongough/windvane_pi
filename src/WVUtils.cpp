...
#ifndef __WXMSW__
    if(g_iLocaleDepth == 0) { 
        g_WVLocale = new wxString(wxSetlocale(LC_NUMERIC, NULL));
...
#if wxCHECK_VERSION(3,0,0)        
        wxSetlocale(LC_NUMERIC, g_WVLocale->ToAscii());
#else
        setlocale(LC_NUMERIC, g_WVLocale->ToAscii());
#endif
        delete g_WVLocale;
        g_WVLocale = NULL;
    } 
